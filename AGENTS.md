...
- Manual fallback: run the same workflow from GitHub Actions with `workflow_dispatch` and provide an existing tag
...
3. Run `npm run build`.
4. If release behavior changed, update `README.md` release or install instructions.
...
If a pushed tag did not create release assets, check the `Release` workflow run in GitHub Actions before changing manifest URLs.
