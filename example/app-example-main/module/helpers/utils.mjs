...
export const getCommandTarget = (options = {}) => {
    ...
    if (!game.user.isGM) {
        ...
        if (!target && !allowNull) {
            ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noAssignedPlayerCharacter'));
            return null;
        }
    }
    if (!target && !allowNull) {
        ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noSelectedToken'));
        return null;
    }
    if (target && target.type !== 'character') {
        if (!allowNull) {
            ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.onlyUseableByPC'));
        }
        return null;
    }
...
export const getDamageLabel = damage => {
    return game.i18n.localize(`APP_EXAMPLE.GENERAL.Damage.${getDamageKey(damage)}`);
};
...
export async function RefreshFeatures(
    ...
    if (sendNotificationMessage) {
        ui.notifications.info(
            game.i18n.format('APP_EXAMPLE.UI.Notifications.gmMenuRefresh', {
                types: `[${types}]`
            })
        );
    }
    ...
    if (sendRefreshMessage) {
        ...
        const msg = {
            ...
            content: await foundry.applications.handlebars.renderTemplate(
                'systems/app-example/templates/ui/chat/refreshMessage.hbs',
                {
                    types: types
                }
            ),
            title: game.i18n.localize('APP_EXAMPLE.UI.Chat.refreshMessage.title'),
