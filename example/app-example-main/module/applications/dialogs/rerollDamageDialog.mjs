...
export default class RerollDamageDialog extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        id: 'reroll-dialog',
        classes: ['app-example', 'dialog', 'dh-style', 'views', 'reroll-dialog'],
        window: {
    ...
    static PARTS = {
        main: {
            id: 'main',
            template: 'systems/app-example/templates/dialogs/rerollDialog/damage/main.hbs'
        },
        footer: {
            id: 'footer',
            template: 'systems/app-example/templates/dialogs/rerollDialog/footer.hbs'
        }
    };

    get title() {
        return game.i18n.localize('APP_EXAMPLE.APPLICATIONS.RerollDialog.damageTitle');
    }
    ...
    static async #selectRoll(_, button) {
        ...
        if (!diceResult.active && diceVal.results.filter(x => x.active).length === diceVal.maxSelected) {
            return ui.notifications.warn(
                game.i18n.localize('APP_EXAMPLE.APPLICATIONS.RerollDialog.deselectDiceNotification')
            );
        }
