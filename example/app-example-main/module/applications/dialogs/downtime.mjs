...
export default class DhpDowntime extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        tag: 'form',
        classes: ['app-example', 'views', 'dh-style', 'dialog', 'downtime'],
    ...
    static PARTS = {
        application: {
            id: 'downtime',
            template: 'systems/app-example/templates/dialogs/downtime/downtime.hbs'
        }
    };
    ...
    async _prepareContext(_options) {
        ...
        context.title = game.i18n.localize(
            `APP_EXAMPLE.APPLICATIONS.Downtime.${this.shortrest ? 'shortRest' : 'longRest'}.title`
        );
    ...
    static selectMove(_, target) {
        ...
        if (nrSelected + this.nrChoices[category].taken >= this.nrChoices[category].max) {
            ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noMoreMoves'));
            return;
        }
    ...
    static async takeDowntime() {
        ...
        const msg = {
            ...
            content: await foundry.applications.handlebars.renderTemplate(
                'systems/app-example/templates/ui/chat/downtime.hbs',
                {
                    title: game.i18n.localize(
                        `APP_EXAMPLE.APPLICATIONS.Downtime.${this.shortrest ? 'shortRest' : 'longRest'}.title`
                    ),
