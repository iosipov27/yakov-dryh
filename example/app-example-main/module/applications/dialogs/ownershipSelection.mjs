...
export default class OwnershipSelection extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        tag: 'form',
        classes: ['app-example', 'views', 'dialog', 'dh-style', 'ownership-selection'],
        window: {
    ...
    static PARTS = {
        selection: {
            template: 'systems/app-example/templates/dialogs/ownershipSelection.hbs'
        }
    };

    get title() {
        return game.i18n.format('APP_EXAMPLE.APPLICATIONS.OwnershipSelection.title', { name: this.name });
    }
