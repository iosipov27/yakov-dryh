...
export default class DhDeathMove extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    get title() {
        return game.i18n.format('APP_EXAMPLE.APPLICATIONS.DeathMove.title', { actor: this.actor.name });
    }

    static DEFAULT_OPTIONS = {
        classes: ['app-example', 'dh-style', 'dialog', 'views', 'death-move'],
    ...
    static PARTS = {
        application: {
            id: 'death-move',
            template: 'systems/app-example/templates/dialogs/deathMove.hbs'
        }
    };
    ...
    async _prepareContext(_options) {
        ...
        context.title = game.i18n.localize('APP_EXAMPLE.APPLICATIONS.DeathMove.takeMove');
    ...
    async handleAvoidDeath(useAutomation) {
        ...
        const config = await enrichedFateRoll({
            target,
            title: game.i18n.localize('APP_EXAMPLE.CONFIG.DeathMoves.avoidDeath.name'),
            label: `${game.i18n.localize('APP_EXAMPLE.GENERAL.hope')} ${game.i18n.localize('APP_EXAMPLE.GENERAL.fateRoll')}`,
            fateType: 'Hope'
        });
        ...
        let returnMessage = game.i18n.localize('APP_EXAMPLE.UI.Chat.deathMove.avoidScar');
        if (config.roll.fate.value <= this.actor.system.levelData.level.current) {
            ...
            if (newScarAmount >= this.actor.system.resources.hope.max) {
                ...
                return game.i18n.format('APP_EXAMPLE.UI.Chat.deathMove.journeysEnd', { scars: newScarAmount });
            }

            returnMessage = game.i18n.localize('APP_EXAMPLE.UI.Chat.deathMove.gainScar');
        }
    ...
    async handleRiskItAll(useAutomation) {
        const config = await enrichedDualityRoll({
            ...
            target: this.actor,
            difficulty: null,
            title: game.i18n.localize('APP_EXAMPLE.CONFIG.DeathMoves.riskItAll.name'),
            label: game.i18n.localize('APP_EXAMPLE.GENERAL.dualityDice'),
            actionType: null,
            advantage: null,
        ...
        if (config.roll.isCritical) {
            ...
            chatMessage = game.i18n.localize('APP_EXAMPLE.UI.Chat.deathMove.riskItAllCritical');
        }
        ...
        if (config.roll.result.duality == 1) {
            if (
                ...
                config.resourceUpdates.addResources(clearAllStressAndHitpointsUpdates);
                chatMessage = game.i18n.localize('APP_EXAMPLE.UI.Chat.deathMove.riskItAllSuccessWithEnoughHope');
            } else {
                chatMessage = game.i18n.format('APP_EXAMPLE.UI.Chat.deathMove.riskItAllSuccess', {
                    hope: config.roll.hope.value
                });
                this.showRiskItAllButton = true;
                this.riskItAllHope = config.roll.hope.value;
                this.riskItAllButtonLabel = game.i18n.format('APP_EXAMPLE.UI.Chat.deathMove.riskItAllDialogButton');
            }
        }

        if (config.roll.result.duality == -1) {
            await this.actor.setDeathMoveDefeated(CONFIG.DH.GENERAL.defeatedConditionChoices.dead.id);
            chatMessage = game.i18n.localize('APP_EXAMPLE.UI.Chat.deathMove.riskItAllFailure');
        }
    ...
    async handleBlazeOfGlory(useAutomation) {
        ...
        this.actor.createEmbeddedDocuments('ActiveEffect', [
            {
                name: game.i18n.localize('APP_EXAMPLE.CONFIG.DeathMoves.blazeOfGlory.name'),
                description: game.i18n.localize('APP_EXAMPLE.CONFIG.DeathMoves.blazeOfGlory.description'),
        ...
        await this.actor.setDeathMoveDefeated(CONFIG.DH.GENERAL.defeatedConditionChoices.dead.id);
        return game.i18n.localize('APP_EXAMPLE.UI.Chat.deathMove.blazeOfGlory');
    }
    ...
    static async takeMove() {
        ...
        const msg = {
            ...
            content: await foundry.applications.handlebars.renderTemplate(
                'systems/app-example/templates/ui/chat/deathMove.hbs',
                {
            ...
            title: game.i18n.localize('APP_EXAMPLE.UI.Chat.deathMove.title'),
