...
export default class BeastformDialog extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        tag: 'form',
        classes: ['app-example', 'views', 'dialog', 'dh-style', 'beastform-selection'],
        position: {
            width: 600,
            height: 'auto'
        },
    ...
    /** @override */
    static PARTS = {
        header: { template: 'systems/app-example/templates/dialogs/beastform/header.hbs' },
        tabs: { template: 'systems/app-example/templates/dialogs/beastform/tabs.hbs' },
        beastformTier: { template: 'systems/app-example/templates/dialogs/beastform/beastformTier.hbs' },
        advanced: { template: 'systems/app-example/templates/dialogs/beastform/advanced.hbs' },
        footer: { template: 'systems/app-example/templates/dialogs/beastform/footer.hbs' }
    };
    ...
    static TABS = {
        primary: {
            ...
            initial: '1',
            labelPrefix: 'APP_EXAMPLE.GENERAL.Tiers'
        }
    };
    ...
    async _prepareContext(_options) {
        ...
        const compendiumBeastforms = await game.packs.get(`app-example.beastforms`)?.getDocuments();
    ...
    static toggleHybridFeature(_, button) {
        ...
        if (this.hybrid.features[button.dataset.form][button.id])
            ...
            if (currentFeatures === this.selected.system.hybrid.features) {
                ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.beastformToManyFeatures'));
                return;
            }
    ...
    static toggleHybridAdvantage(_, button) {
        ...
        if (this.hybrid.advantages[button.dataset.form][button.id])
            ...
            if (currentAdvantages === this.selected.system.hybrid.advantages) {
                ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.beastformToManyAdvantages'));
                return;
            }
