...
export default class DamageReductionDialog extends HandlebarsApplicationMixin(ApplicationV2) {
        ...
        this.thresholdImmunities = Object.keys(actor.system.rules.damageReduction.thresholdImmunities).reduce(
            (acc, key) => {
                if (actor.system.rules.damageReduction.thresholdImmunities[key])
                    acc[damageKeyToNumber(key)] = game.i18n.format(`APP_EXAMPLE.GENERAL.DamageThresholds.with`, {
                        threshold: game.i18n.localize(`APP_EXAMPLE.GENERAL.DamageThresholds.${key}`)
                    });
                return acc;
            },
            {}
    ...
    static DEFAULT_OPTIONS = {
        tag: 'form',
        classes: ['app-example', 'views', 'damage-reduction'],
        position: {
            width: 280,
            height: 'auto'
        },
    ...
    static PARTS = {
        damageSelection: {
            id: 'damageReduction',
            template: 'systems/app-example/templates/dialogs/damageReduction.hbs'
        }
    };
    ...
    /** @inheritDoc */
    get title() {
        return game.i18n.localize('APP_EXAMPLE.APPLICATIONS.DamageReduction.title');
    }
    ...
    static setMarks(_, target) {
        ...
        if (!currentMark.selected && currentDamage === 0) {
            ui.notifications.info(game.i18n.localize('APP_EXAMPLE.UI.Notifications.damageAlreadyNone'));
            return;
        }

        if (this.rulesOn) {
            if (!currentMark.selected && currentMarks === this.actor.system.armorScore) {
                ui.notifications.info(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noAvailableArmorMarks'));
                return;
            }
        }
    ...
    static useStressReduction(_, target) {
        ...
        if (stressReduction.selected) {
            ...
            if (currentStress + stressReduction.cost > this.actor.system.resources.stress.max) {
                ui.notifications.info(game.i18n.localize('APP_EXAMPLE.UI.Notifications.notEnoughStress'));
                return;
            }
