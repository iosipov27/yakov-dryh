export default class DhHotbar extends foundry.applications.ui.Hotbar {
    ...
    static async useAction(itemUuid, actionId) {
        ...
        if (!action) {
            return ui.notifications.warn('APP_EXAMPLE.UI.Notifications.actionIsMissing');
        }
    ...
    static async useAttack(actorUuid) {
        ...
        if (!attack) {
            return ui.notifications.warn('APP_EXAMPLE.UI.Notifications.attackIsMissing');
        }
        ...
        Hooks.on('hotbarDrop', (bar, data, slot) => {
            if (data.type === 'Item') {
                ...
                if (!item.isOwned || !item.isOwner) {
                    ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.unownedActionMacro'));
                    return false;
                }
                ...
                if (!actor.isOwner) {
                    ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.unownedAttackMacro'));
                    return false;
                }
