...
export class ItemBrowser extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        id: 'itemBrowser',
        classes: ['app-example', 'dh-style', 'dialog', 'compendium-browser', 'app-example-loader'],
        tag: 'div',
        window: {
            frame: true,
    ...
    static PARTS = {
        sidebar: {
            template: 'systems/app-example/templates/ui/itemBrowser/sidebar.hbs'
        },
        list: {
            template: 'systems/app-example/templates/ui/itemBrowser/itemBrowser.hbs'
        }
    };
    ...
    loadItems() {
        ...
        Promise.all(promises).then(async result => {
            ...
            for (const item of this.items) {
                if (['weapon', 'armor'].includes(item.type)) {
                    item.system.enrichedTags = await foundry.applications.handlebars.renderTemplate(
                        'systems/app-example/templates/ui/itemBrowser/item-tags.hbs',
                        { item: item.system }
                    );
                }
            ...
            const filterList = await foundry.applications.handlebars.renderTemplate(
                'systems/app-example/templates/ui/itemBrowser/filterContainer.hbs',
                {
    ...
    toggleLoader(state) {
        ...
        return setTimeout(() => {
            container.classList.toggle('app-example-loader', state);
        }, 100);
    }
    ...
    static injectSidebarButton(html) {
        ...
        if (Object.keys(menus).includes(sectionId)) {
            ...
            button.innerHTML = `
                ...
                ${game.i18n.localize('APP_EXAMPLE.UI.Tooltip.compendiumBrowser')}
            `;
