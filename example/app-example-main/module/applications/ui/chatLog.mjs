...
export default class DhpChatLog extends foundry.applications.sidebar.tabs.ChatLog {
    ...
    _getEntryContextOptions() {
        return [
            ...
            // },
            {
                name: game.i18n.localize('APP_EXAMPLE.UI.ChatLog.rerollDamage'),
                icon: '<i class="fa-solid fa-dice"></i>',
                condition: li => {
                    const message = game.messages.get(li.dataset.messageId);
    ...
    async onRollSimple(event, message) {
        ...
        if (targets.length === 0)
            return ui.notifications.info(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noTargetsSelected'));
    ...
    async rerollEvent(event, message) {
        ...
        if (!event.shiftKey) {
            const confirmed = await foundry.applications.api.DialogV2.confirm({
                window: {
                    title: game.i18n.localize('APP_EXAMPLE.UI.Chat.reroll.confirmTitle')
                },
                content: game.i18n.localize('APP_EXAMPLE.UI.Chat.reroll.confirmText')
            });
            if (!confirmed) return;
        }
    ...
    async groupRollButton(event, message) {
        ...
        if (!actor) {
            return ui.notifications.error(
                game.i18n.format('APP_EXAMPLE.UI.Notifications.documentIsMissing', {
        ...
        if (!actor.testUserPermission(game.user, 'OWNER')) {
            return ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noActorOwnership'));
        }

        const traitLabel = game.i18n.localize(abilities[trait].label);
        const config = {
            event: event,
            title: `${game.i18n.localize('APP_EXAMPLE.GENERAL.dualityRoll')}: ${actor.name}`,
        ...
        const result = await actor.diceRoll({
            ...config,
            headerTitle: `${game.i18n.localize('APP_EXAMPLE.GENERAL.dualityRoll')}: ${actor.name}`,
        ...
        const updatedContent = await foundry.applications.handlebars.renderTemplate(
            'systems/app-example/templates/ui/chat/groupRoll.hbs',
    ...
    async groupRollSuccessEvent(event, message) {
        if (!game.user.isGM) {
            return ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.gmOnly'));
        }
