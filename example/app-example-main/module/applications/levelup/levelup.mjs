...
export default class DhlevelUp extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    get title() {
        return game.i18n.format('APP_EXAMPLE.APPLICATIONS.Levelup.title', { actor: this.actor.name });
    }

    static DEFAULT_OPTIONS = {
        tag: 'form',
        classes: ['app-example', 'dialog', 'dh-style', 'levelup'],
    ...
    static PARTS = {
        tabs: { template: 'systems/app-example/templates/levelup/tabs/tab-navigation.hbs' },
        advancements: {
            template: 'systems/app-example/templates/levelup/tabs/advancements.hbs'
        },
        selections: {
            template: 'systems/app-example/templates/levelup/tabs/selections.hbs',
            scrollable: ['.levelup-selections-container']
        },
        summary: { template: 'systems/app-example/templates/levelup/tabs/summary.hbs' },
        footer: { template: 'systems/app-example/templates/levelup/tabs/footer.hbs' }
    };
    ...
    static TABS = {
        advancements: {
            ...
            id: 'advancements',
            icon: null,
            label: 'APP_EXAMPLE.GENERAL.Tabs.advancement'
        },
        selections: {
            active: false,
            cssClass: '',
            ...
            id: 'selections',
            icon: null,
            label: 'APP_EXAMPLE.GENERAL.Tabs.selections'
        },
        summary: {
            active: false,
            cssClass: '',
            ...
            id: 'summary',
            icon: null,
            label: 'APP_EXAMPLE.GENERAL.Tabs.summary'
        }
    };
            ...
            case 'tabs':
                ...
                context.navigate = {
                    previous: {
                        disabled: !previous,
                        label: previous
                            ? game.i18n.format('APP_EXAMPLE.APPLICATIONS.Levelup.navigateLevel', { level: previous })
                            : '',
                    ...
                    next: {
                        ...
                        label: next
                            ? game.i18n.format('APP_EXAMPLE.APPLICATIONS.Levelup.navigateLevel', { level: next })
                            : '',
                        toSummary: !next,
    ...
    tagifyUpdate =
        ...
        async (_, { option, removed }) => {
            ...
            if (!updatePath) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noSelectionsLeft'));
                return;
            }
        ...
        if (event.target.closest('.domain-cards')) {
            ...
            if (item.type === 'domainCard') {
                ...
                if (
                    ...
                    ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.domainCardWrongDomain'));
                    return;
                }
                ...
                if (levelMax < item.system.level) {
                    ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.domainCardToHighLevel'));
                    return;
                }
                ...
                if (cardExistsInCharacter || cardExistsInLevelup) {
                    ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.domainCardDuplicate'));
                    return;
                }
            ...
            if (item.type === 'class') {
                if (item.name === this.actor.system.class.value.name) {
                    ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.alreadySelectedClass'));
                    return;
                }
    ...
    async selectionClick(event) {
        ...
        if (!button.checked) {
            ...
            if (this.levelup.levels[this.levelup.currentLevel].nrSelections.available < Number(button.dataset.cost)) {
                ui.notifications.info(game.i18n.localize('APP_EXAMPLE.UI.Notifications.insufficentAdvancements'));
                this.render();
                return;
            }
    ...
    static async updateCurrentLevel(_, button) {
        if (!button.dataset.forward) {
            const confirmed = await foundry.applications.api.DialogV2.confirm({
                window: {
                    title: game.i18n.localize('APP_EXAMPLE.APPLICATIONS.Levelup.delevel.title')
                },
                content: game.i18n.format('APP_EXAMPLE.APPLICATIONS.Levelup.delevel.content')
            });
