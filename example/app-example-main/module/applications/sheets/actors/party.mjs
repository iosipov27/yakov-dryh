...
import AppExampleMenu from '../../sidebar/tabs/appExampleMenu.mjs';
...
export default class Party extends DHBaseActorSheet {
    ...
    /**@override */
    static PARTS = {
        header: { template: 'systems/app-example/templates/sheets/actors/party/header.hbs' },
        tabs: { template: 'systems/app-example/templates/sheets/global/tabs/tab-navigation.hbs' },
        partyMembers: { template: 'systems/app-example/templates/sheets/actors/party/party-members.hbs' },
        resources: {
            template: 'systems/app-example/templates/sheets/actors/party/resources.hbs',
            scrollable: ['']
        },
        /* NOT YET IMPLEMENTED */
        // projects: {
        //     template: 'systems/app-example/templates/sheets/actors/party/projects.hbs',
        //     scrollable: ['']
        // },
        inventory: {
            template: 'systems/app-example/templates/sheets/actors/party/inventory.hbs',
            scrollable: ['.tab.inventory .items-section']
        },
        notes: { template: 'systems/app-example/templates/sheets/actors/party/notes.hbs' }
    };
    ...
    static async #refeshActions() {
        const confirmed = await foundry.applications.api.DialogV2.confirm({
            ...
            content: await foundry.applications.handlebars.renderTemplate(
                'systems/app-example/templates/sidebar/app-example-menu/main.hbs',
                {
                    refreshables: AppExampleMenu.defaultRefreshSelections()
                }
            ),
            classes: ['app-example', 'dialog', 'dh-style', 'tab', 'sidebar-tab', 'appExampleMenu-sidebar']
        });
    ...
    static async #triggerRest(_, button) {
        const confirmed = await foundry.applications.api.DialogV2.confirm({
            window: {
                title: game.i18n.localize(`APP_EXAMPLE.APPLICATIONS.Downtime.${button.dataset.type}.title`),
            ...
            classes: ['app-example', 'dialog', 'dh-style']
        });
    ...
    async _onDropActor(event, document) {
        ...
        if (document instanceof DhpActor && Party.ALLOWED_ACTOR_TYPES.includes(document.type)) {
            ...
            if (currentMembers.includes(data.uuid)) {
                return ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.duplicateCharacter'));
            }

            await this.document.update({ 'system.partyMembers': [...currentMembers, document.uuid] });
        } else {
            ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.onlyCharactersInPartySheet'));
        }
    ...
    static async #deletePartyMember(event, target) {
        ...
        if (!event.shiftKey) {
            const confirmed = await foundry.applications.api.DialogV2.confirm({
                ...
                content: game.i18n.format('APP_EXAMPLE.APPLICATIONS.DeleteConfirmation.text', { name: doc.name })
            });
