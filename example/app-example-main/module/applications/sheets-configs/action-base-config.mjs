import AppExampleSheet from '../sheets/app-example-sheet.mjs';

const { ApplicationV2 } = foundry.applications.api;
export default class DHActionBaseConfig extends AppExampleSheet(ApplicationV2) {
    constructor(action) {
        super({});
    ...
    get title() {
        return `${game.i18n.localize('APP_EXAMPLE.GENERAL.Tabs.settings')}: ${this.action.name}`;
    }

    static DEFAULT_OPTIONS = {
        tag: 'form',
        classes: ['app-example', 'dh-style', 'action-config', 'dialog', 'max-800'],
        window: {
    ...
    static PARTS = {
        header: {
            id: 'header',
            template: 'systems/app-example/templates/sheets-settings/action-settings/header.hbs'
        },
        tabs: { template: 'systems/app-example/templates/sheets/global/tabs/tab-navigation.hbs' },
        base: {
            id: 'base',
            template: 'systems/app-example/templates/sheets-settings/action-settings/base.hbs'
        },
        configuration: {
            id: 'configuration',
            template: 'systems/app-example/templates/sheets-settings/action-settings/configuration.hbs'
        },
        effect: {
            id: 'effect',
            template: 'systems/app-example/templates/sheets-settings/action-settings/effect.hbs'
        },
        trigger: {
            id: 'trigger',
            template: 'systems/app-example/templates/sheets-settings/action-settings/trigger.hbs'
        }
    };
    ...
    static TABS = {
        base: {
            ...
            id: 'base',
            icon: null,
            label: 'APP_EXAMPLE.GENERAL.Tabs.base'
        },
        config: {
            active: false,
            cssClass: '',
            ...
            id: 'config',
            icon: null,
            label: 'APP_EXAMPLE.GENERAL.Tabs.configuration'
        },
        effect: {
            active: false,
            cssClass: '',
            ...
            id: 'effect',
            icon: null,
            label: 'APP_EXAMPLE.GENERAL.Tabs.effects'
        },
        trigger: {
            active: false,
            cssClass: '',
            ...
            id: 'trigger',
            icon: null,
            label: 'APP_EXAMPLE.GENERAL.Tabs.triggers'
        }
    };
    ...
    async _prepareContext(_options) {
            ...
            context.transform = {
                ...
                actor:
                    ...
                    (context.source.transform.actorUUID && !actor
                        ? { error: game.i18n.localize('APP_EXAMPLE.ACTIONS.Settings.transform.actorIsMissing') }
                        : null)
            };
        }
        ...
        context.tierOptions = [
            { key: 1, label: game.i18n.localize('APP_EXAMPLE.GENERAL.Tiers.1') },
    ...
    getCostOptions() {
        ...
        if (resource) {
            options.resource = {
                label: 'APP_EXAMPLE.GENERAL.itemResource',
                group: 'Global'
            };
        }
        ...
        if (this.action.parent.metadata?.isQuantifiable) {
            options.quantity = {
                label: 'APP_EXAMPLE.GENERAL.itemQuantity',
                group: 'Global'
            };
        }
    ...
    static async removeTrigger(_event, button) {
        ...
        const confirmed = await foundry.applications.api.DialogV2.confirm({
            window: {
                title: game.i18n.localize('APP_EXAMPLE.ACTIONS.Config.deleteTriggerTitle')
            },
            content: game.i18n.format('APP_EXAMPLE.ACTIONS.Config.deleteTriggerContent', {
    ...
    async _onDrop(event) {
        ...
        if (!(item instanceof game.system.api.documents.DhpActor)) {
            ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.ACTIONS.TYPES.summon.invalidDrop'));
            return;
        }
