...
export default class DHCharacterSettings extends DHBaseActorSettings {
    ...
    static PARTS = {
        header: {
            id: 'header',
            template: 'systems/app-example/templates/sheets-settings/character-settings/header.hbs'
        },
        tabs: { template: 'systems/app-example/templates/sheets/global/tabs/tab-navigation.hbs' },
        details: {
            id: 'details',
            template: 'systems/app-example/templates/sheets-settings/character-settings/details.hbs'
        },
        experiences: {
            id: 'experiences',
            template: 'systems/app-example/templates/sheets-settings/character-settings/experiences.hbs'
        }
    };
    ...
    static async #removeExperience(_, target) {
        ...
        if (relinkAchievementData.length > 0 || relinkSelectionData.length > 0) {
            const confirmed = await foundry.applications.api.DialogV2.confirm({
                window: {
                    title: game.i18n.localize('APP_EXAMPLE.ACTORS.Character.experienceDataRemoveConfirmation.title')
                },
                content: game.i18n.localize('APP_EXAMPLE.ACTORS.Character.experienceDataRemoveConfirmation.text')
            });
            if (!confirmed) return;
        }
            ...
            const confirmed = await foundry.applications.api.DialogV2.confirm({
                window: {
                    title: game.i18n.format('APP_EXAMPLE.APPLICATIONS.DeleteConfirmation.title', {
                        type: game.i18n.localize(`APP_EXAMPLE.GENERAL.Experience.single`),
                        name: experience.name
                    })
                },
                content: game.i18n.format('APP_EXAMPLE.APPLICATIONS.DeleteConfirmation.text', { name: experience.name })
            });
            if (!confirmed) return;
        }
