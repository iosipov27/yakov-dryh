...
export default class DHTokenHUD extends foundry.applications.hud.TokenHUD {
    ...
    static PARTS = {
        hud: {
            root: true,
            template: 'systems/app-example/templates/hud/tokenHUD.hbs'
        }
    };
    ...
    async _prepareContext(options) {
        ...
        context.icons.toggleClowncar = 'systems/app-example/assets/icons/arrow-dunk.png';
    ...
    static async #onToggleCombat() {
        ...
        const warning =
            tokensWithoutActors.length === 1
                ? game.i18n.format('APP_EXAMPLE.UI.Notifications.tokenActorMissing', {
                      name: tokensWithoutActors[0].name
                  })
                : game.i18n.format('APP_EXAMPLE.UI.Notifications.tokenActorsMissing', {
    ...
    static async #togglePartyTokens(_, button) {
        ...
        button.dataset.tooltip = game.i18n.localize(
            icon.classList.contains('flipped')
                ? 'APP_EXAMPLE.APPLICATIONS.HUD.tokenHUD.retrievePartyTokens'
                : 'APP_EXAMPLE.APPLICATIONS.HUD.tokenHUD.depositPartyTokens'
        );
    ...
    static async #toggleCompanions(_, button) {
        ...
        button.dataset.tooltip = game.i18n.localize(
            icon.classList.contains('flipped')
                ? 'APP_EXAMPLE.APPLICATIONS.HUD.tokenHUD.retrieveCompanionTokens'
                : 'APP_EXAMPLE.APPLICATIONS.HUD.tokenHUD.depositCompanionTokens'
        );
