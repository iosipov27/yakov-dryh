...
export default class DhCharacterCreation extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    get title() {
        return game.i18n.format('APP_EXAMPLE.APPLICATIONS.CharacterCreation.title', { actor: this.character.name });
    }

    static DEFAULT_OPTIONS = {
        tag: 'form',
        classes: ['app-example', 'dialog', 'dh-style', 'character-creation'],
    ...
    static PARTS = {
        tabs: { template: 'systems/app-example/templates/characterCreation/tabs.hbs' },
        class: { template: 'systems/app-example/templates/characterCreation/tabs/class.hbs' },
        ancestry: { template: 'systems/app-example/templates/characterCreation/tabs/ancestry.hbs' },
        community: { template: 'systems/app-example/templates/characterCreation/tabs/community.hbs' },
        traits: { template: 'systems/app-example/templates/characterCreation/tabs/traits.hbs' },
        experience: { template: 'systems/app-example/templates/characterCreation/tabs/experience.hbs' },
        domainCards: { template: 'systems/app-example/templates/characterCreation/tabs/domainCards.hbs' },
        equipment: { template: 'systems/app-example/templates/characterCreation/equipment.hbs' },
        // story: { template: 'systems/app-example/templates/characterCreation/story.hbs' },
        footer: { template: 'systems/app-example/templates/characterCreation/footer.hbs' }
    };
    ...
    static TABS = {
        class: {
            ...
            group: 'setup',
            id: 'class',
            label: 'APP_EXAMPLE.APPLICATIONS.CharacterCreation.tabs.class'
        },
        ancestry: {
            active: true,
            cssClass: '',
            group: 'setup',
            id: 'ancestry',
            label: 'APP_EXAMPLE.APPLICATIONS.CharacterCreation.tabs.ancestry'
        },
        community: {
            active: false,
            cssClass: '',
            group: 'setup',
            id: 'community',
            label: 'APP_EXAMPLE.APPLICATIONS.CharacterCreation.tabs.community'
        },
        traits: {
            active: false,
            cssClass: '',
            group: 'setup',
            id: 'traits',
            label: 'APP_EXAMPLE.APPLICATIONS.CharacterCreation.tabs.traits'
        },
        experience: {
            active: false,
            cssClass: '',
            group: 'setup',
            id: 'experience',
            label: 'APP_EXAMPLE.APPLICATIONS.CharacterCreation.tabs.experience'
        },
        domainCards: {
            active: false,
            cssClass: '',
            group: 'setup',
            id: 'domainCards',
            label: 'APP_EXAMPLE.APPLICATIONS.CharacterCreation.tabs.domainCards'
        },
        equipment: {
            active: false,
            cssClass: '',
            group: 'setup',
            id: 'equipment',
            label: 'APP_EXAMPLE.APPLICATIONS.CharacterCreation.tabs.equipment'
        }
    };
    ...
    async _prepareContext(_options) {
        ...
        context.suggestedTraits = this.setup.class.system
            ? Object.keys(this.setup.class.system.characterGuide.suggestedTraits).map(traitKey => {
                  ...
                  return `${game.i18n.localize(`APP_EXAMPLE.CONFIG.Traits.${traitKey}.short`)} ${trait > 0 ? `+${trait}` : trait}`;
              })
            : [];
        context.traits = {
    ...
    async _onDrop(event) {
        ...
        if (item.type === 'ancestry' && event.target.closest('.primary-ancestry-card')) {
            ...
            if (this.setup.class.system.subclasses.every(subclass => subclass.uuid !== item.uuid)) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.subclassNotInClass'));
                return;
            }
            ...
            if (!this.setup.class.uuid) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.missingClass'));
                return;
            }

            if (!this.setup.class.system.domains.includes(item.system.domain)) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.wrongDomain'));
                return;
            }

            if (item.system.level > 1) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.cardTooHighLevel'));
                return;
            }

            if (Object.values(this.setup.domainCards).some(card => card.uuid === item.uuid)) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.duplicateCard'));
                return;
            }
            ...
            if (item.system.tier > 1) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.itemTooHighTier'));
                return;
            }
            ...
            if (item.system.secondary) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.notPrimary'));
                return;
            }
            ...
            if (this.equipment.primaryWeapon?.system?.burden === burden.twoHanded.value) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.primaryIsTwoHanded'));
                return;
            }

            if (!item.system.secondary) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.notSecondary'));
                return;
            }
