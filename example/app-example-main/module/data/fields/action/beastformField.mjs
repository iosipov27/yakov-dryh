...
export default class BeastformField extends fields.SchemaField {
                ...
                exact: new fields.NumberField({
                    ...
                    choices: () => {
                        ...
                        return Object.values(settingsTiers).reduce(
                            ...
                            { 1: game.i18n.localize('APP_EXAMPLE.GENERAL.Tiers.1') }
                        );
                    },
                    hint: 'APP_EXAMPLE.ACTIONS.Config.beastform.exactHint'
                })
            })
        };
    ...
    prepareConfig(config) {
        if (this.actor.effects.find(x => x.type === 'beastform')) {
            ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.beastformAlreadyApplied'));
            return false;
        }
    ...
    static async transform(selectedForm, evolvedData, hybridData) {
        ...
        if (!beastformEffect) {
            ui.notifications.error('APP_EXAMPLE.UI.Notifications.beastformMissingEffect');
            return false;
        }
