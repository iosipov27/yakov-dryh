...
export default class DHSummonField extends fields.SchemaField {
    ...
    static async execute() {
        if (!this.transform.actorUUID) {
            ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.ACTIONS.TYPES.transform.noTransformActor'));
            return false;
        }

        const baseActor = await foundry.utils.fromUuid(this.transform.actorUUID);
        if (!baseActor) {
            ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.ACTIONS.TYPES.transform.transformActorMissing'));
            return false;
        }

        if (!canvas.scene) {
            ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.ACTIONS.TYPES.transform.canvasError'));
            return false;
        }

        if (this.actor.prototypeToken.actorLink) {
            ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.ACTIONS.TYPES.transform.actorLinkError'));
            return false;
        }

        if (!this.actor.token) {
            ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.ACTIONS.TYPES.transform.prototypeError'));
            return false;
        }
