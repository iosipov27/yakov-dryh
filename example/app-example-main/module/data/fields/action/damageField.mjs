...
export default class DamageField extends fields.SchemaField {
    ...
    constructor(options, context = {}) {
        const damageFields = {
            ...
            includeBase: new fields.BooleanField({
                initial: false,
                label: 'APP_EXAMPLE.ACTIONS.Settings.includeBase.label'
            }),
            direct: new fields.BooleanField({ initial: false, label: 'APP_EXAMPLE.CONFIG.DamageType.direct.name' })
        };
    ...
    static async execute(config, messageId = null, force = false) {
        ...
        const damageResult = await CONFIG.Dice["app-example"].DamageRoll.build(damageConfig);
        ...
        Promise.all(damagePromises).then(async _ => {
            ...
            const msg = {
                ...
                title: game.i18n.localize(
                    `APP_EXAMPLE.UI.Chat.damageSummary.${config.hasHealing ? 'healingTitle' : 'title'}`
                ),
                content: await foundry.applications.handlebars.renderTemplate(
                    'systems/app-example/templates/ui/chat/damageSummary.hbs',
                    {
    ...
    static defineSchema() {
        return {
            multiplier: new fields.StringField({
                ...
                initial: 'prof',
                label: 'APP_EXAMPLE.ACTIONS.Config.damage.multiplier',
                nullable: false,
                required: true
            }),
            flatMultiplier: new fields.NumberField({
                nullable: true,
                initial: 1,
                label: 'APP_EXAMPLE.ACTIONS.Config.damage.flatMultiplier'
            }),
            dice: new fields.StringField({
                choices: CONFIG.DH.GENERAL.diceTypes,
                initial: 'd6',
                label: 'APP_EXAMPLE.GENERAL.Dice.single',
                nullable: false,
                required: true
            }),
            bonus: new fields.NumberField({ nullable: true, initial: null, label: 'APP_EXAMPLE.GENERAL.bonus' }),
            custom: new fields.SchemaField({
                enabled: new fields.BooleanField({ label: 'APP_EXAMPLE.ACTIONS.Config.general.customFormula' }),
                formula: new FormulaField({ label: 'APP_EXAMPLE.ACTIONS.Config.general.formula', initial: '' })
            })
        };
    }
    ...
    static defineSchema() {
        return {
            applyTo: new fields.StringField({
                ...
                label: 'APP_EXAMPLE.ACTIONS.Settings.applyTo.label'
            }),
            resultBased: new fields.BooleanField({
                initial: false,
                label: 'APP_EXAMPLE.ACTIONS.Settings.resultBased.label'
            }),
