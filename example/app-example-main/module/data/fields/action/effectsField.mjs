...
export default class EffectsField extends fields.ArrayField {
    ...
    static async execute(config, targets = null, force = false) {
        ...
        if (!message) {
            const roll = new CONFIG.Dice["app-example"].DHRoll('');
            roll._evaluated = true;
            message = config.message = await CONFIG.Dice["app-example"].DHRoll.toMessage(roll, config);
        }
    ...
    static async applyEffects(targets) {
        ...
        targets.forEach(async baseToken => {
            ...
            messageTargets.push({
                ...
                conditionImmunities: Object.values(conditionImmunities).some(x => x)
                    ? game.i18n.format('APP_EXAMPLE.UI.Chat.effectSummary.immunityTo', {
        ...
        const msg = {
            ...
            title: game.i18n.localize('APP_EXAMPLE.UI.Chat.effectSummary.title'),
            content: await foundry.applications.handlebars.renderTemplate(
                'systems/app-example/templates/ui/chat/effectSummary.hbs',
                {
