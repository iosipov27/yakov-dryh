...
export default class DHClass extends BaseDataItem {
    ...
    /**@override */
    static DEFAULT_ICON = 'systems/app-example/assets/icons/documents/items/laurel-crown.svg';
        ...
        if (this.actor?.type === 'character') {
            ...
            if (levelupAuto) {
                ...
                if (foundry.utils.getProperty(this.actor, path)) {
                    ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.classAlreadySelected'));
                    return false;
                }
            } else {
                if (this.actor.system.class.value) {
                    if (this.actor.system.multiclass.value) {
                        ui.notifications.warn(
                            game.i18n.localize('APP_EXAMPLE.UI.Notifications.multiclassAlreadyPresent')
                        );
                        return false;
                    } else {
    ...
    async _preUpdate(changed, options, userId) {
        ...
        if (changed.system?.domains) {
            ...
            if (changed.system.domains.length > maxDomains) {
                ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.domainMaxReached'));
                return false;
            }
        }
    ...
    async getDescriptionData() {
        ...
        if (this.domains.length) {
            if (this.domains.length === 1) domainsLabel = getDomainLabel(this.domains[0]);
                ...
                domainsLabel = game.i18n.format('APP_EXAMPLE.GENERAL.thingsAndThing', {
                    things: firstDomains,
                    thing: lastDomain
                });
            }
        ...
        const suffix = await foundry.applications.handlebars.renderTemplate(
            'systems/app-example/templates/sheets/items/class/description.hbs',
            {
                class: this.parent,
