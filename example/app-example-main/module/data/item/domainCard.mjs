...
export default class DHDomainCard extends BaseDataItem {
    ...
    /**@override */
    static DEFAULT_ICON = 'systems/app-example/assets/icons/documents/items/card-play.svg';
    ...
    async _preCreate(data, options, user) {
        ...
        if (this.actor?.type === 'character') {
            ...
            if (!actorClasses.length) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noClassSelected'));
                return false;
            }

            if (!actorClasses.some(c => c.system.domains.find(x => x === this.domain))) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.lacksDomain'));
                return false;
            }

            if (this.actor.system.domainCards.total.find(x => x.name === this.parent.name)) {
                ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.duplicateDomainCard'));
                return false;
            }
    ...
    _getTags() {
        const tags = [
            game.i18n.localize(`APP_EXAMPLE.CONFIG.DomainCardTypes.${this.type}`),
            this.domainLabel,
            `${game.i18n.localize('APP_EXAMPLE.GENERAL.levelShort')}: ${this.level}`,
            `${game.i18n.localize('APP_EXAMPLE.ITEMS.DomainCard.recallCost')}: ${this.recallCost}`
        ];
    ...
    _getLabels() {
        ...
        if (this.type) labels.push(game.i18n.localize(`APP_EXAMPLE.CONFIG.DomainCardTypes.${this.type}`));
