...
export default class DHSubclass extends BaseDataItem {
    ...
    /**@override */
    static DEFAULT_ICON = 'systems/app-example/assets/icons/documents/items/laurels.svg';
        ...
        if (this.actor?.type === 'character') {
            ...
            if (this.actor.system.class.subclass) {
                if (this.actor.system.multiclass.subclass) {
                    ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.subclassesAlreadyPresent'));
                    return false;
                } else {
                    const multiclass = this.actor.items.find(x => x.type === 'class' && x.system.isMulticlass);
                    if (!multiclass) {
                        ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.missingMulticlass'));
                        return false;
                    }

                    if (multiclass.system.subclasses.every(x => x.uuid !== dataUuid)) {
                        ui.notifications.error(
                            game.i18n.localize('APP_EXAMPLE.UI.Notifications.subclassNotInMulticlass')
                        );
                        return false;
                    }
                ...
                if (!actorClass) {
                    ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.missingClass'));
                    return false;
                }
    ...
    async getDescriptionData() {
        ...
        const suffix = await foundry.applications.handlebars.renderTemplate(
            'systems/app-example/templates/sheets/items/subclass/description.hbs',
            {
                spellcastTrait,
                foundationFeatures,
