...
export default class DHWeapon extends AttachableItem {
    ...
    static defineSchema() {
        ...
        return {
            ...
            //SETTINGS
            secondary: new fields.BooleanField({ initial: false, label: 'APP_EXAMPLE.ITEMS.Weapon.secondaryWeapon' }),
    ...
    /**@override */
    static DEFAULT_ICON = 'systems/app-example/assets/icons/documents/items/battered-axe.svg';
    ...
    async getDescriptionData() {
        ...
        const prefix = await foundry.applications.handlebars.renderTemplate(
            'systems/app-example/templates/sheets/items/weapon/description.hbs',
            {
                item: this,
                features
            }
    ...
    _getTags() {
        ...
        const tags = [
            game.i18n.localize(`APP_EXAMPLE.CONFIG.Traits.${attack.roll.trait}.name`),
            game.i18n.localize(`APP_EXAMPLE.CONFIG.Range.${attack.range}.name`),
            game.i18n.localize(`APP_EXAMPLE.CONFIG.Burden.${burden}`)
        ];

        for (const { value, type } of attack.damage.parts) {
            const parts = value.custom.enabled ? [game.i18n.localize('APP_EXAMPLE.GENERAL.custom')] : [value.dice];
            if (!value.custom.enabled && value.bonus) parts.push(value.bonus.signedString());

            if (type.size > 0) {
                const typeTags = Array.from(type)
                    .map(t => game.i18n.localize(`APP_EXAMPLE.CONFIG.DamageType.${t}.abbreviation`))
                    .join(' | ');
    ...
    _getLabels() {
        ...
        if (roll.trait) labels.push(game.i18n.localize(`APP_EXAMPLE.CONFIG.Traits.${roll.trait}.short`));
        if (range) labels.push(game.i18n.localize(`APP_EXAMPLE.CONFIG.Range.${range}.short`));
