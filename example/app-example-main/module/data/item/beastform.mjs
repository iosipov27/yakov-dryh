...
export default class DHBeastform extends BaseDataItem {
    static LOCALIZATION_PREFIXES = ['APP_EXAMPLE.ITEMS.Beastform'];
        ...
        return {
            ...
            hybrid: new fields.SchemaField({
                maximumTier: new fields.NumberField({
                    integer: true,
                    choices: CONFIG.DH.GENERAL.tiers,
                    label: 'APP_EXAMPLE.ITEMS.Beastform.FIELDS.evolved.maximumTier.label'
                }),
    ...
    /**@override */
    static DEFAULT_ICON = 'systems/app-example/assets/icons/documents/items/wolf-head.svg';
    ...
    static async getWildcardImage(actor, beastform) {
        ...
        if (usesWildcard) {
            ...
            const selectedImage = await game.system.api.applications.dialogs.ImageSelectDialog.configure(
                game.i18n.localize('APP_EXAMPLE.APPLICATIONS.ImageSelect.title'),
                files
            );
    ...
    async _preCreate() {
        ...
        if (this.actor.type !== 'character') {
            ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.beastformInapplicable'));
            return false;
        }

        if (this.actor.items.find(x => x.type === 'beastform')) {
            ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.beastformAlreadyApplied'));
            return false;
        }
        ...
        if (!this.actor && game.user.id === userId) {
            ...
            if (!hasBeastformEffect)
                this.parent.createEmbeddedDocuments('ActiveEffect', [
                    {
                        type: 'beastform',
                        name: game.i18n.localize('APP_EXAMPLE.ITEMS.Beastform.beastformEffect'),
