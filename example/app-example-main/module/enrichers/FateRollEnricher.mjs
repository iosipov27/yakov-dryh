...
export function getFateTypeData(fateTypeValue) {
    ...
    switch (lowercased) {
        case 'hope':
        case 'fear':
            return { value, label: game.i18n.localize(`APP_EXAMPLE.GENERAL.${lowercased}`) };
        default:
            return null;
    }
}
...
export const renderFateButton = async event => {
    ...
    if (!fateTypeData) ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.fateTypeParsing'));
    const { value: fateType, label: fateTypeLabel } = fateTypeData;
...
export const enrichedFateRoll = async ({ target, title, label, fateType }, event) => {
    ...
    await CONFIG.Dice["app-example"].FateRoll.build(config);
    return config;
};
