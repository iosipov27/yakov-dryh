...
export default class DhScene extends Scene {
    ...
    prepareBaseData() {
        ...
        if (this instanceof game.system.api.documents.DhScene) {
            const system = new game.system.api.data.scenes.DHScene(this.flags["app-example"]);
    ...
    async _preUpdate(changes, options, user) {
        ...
        if (changes.flags?.["app-example"]) {
            if (this._source.flags["app-example"]) {
                const unregisterTriggerData = (this._source.flags["app-example"].sceneEnvironments ?? []).reduce(
                    (acc, env) => {
                        if (!changes.flags["app-example"].sceneEnvironments.includes(env)) acc.sceneEnvironments.push(env);

                        return acc;
                    },
                    { ...this._source.flags["app-example"], sceneEnvironments: [] }
                );
                game.system.registeredTriggers.unregisterSceneEnvironmentTriggers(unregisterTriggerData);
            }

            game.system.registeredTriggers.registerSceneEnvironmentTriggers(changes.flags["app-example"]);
        }
    }
