...
export default class DhpActor extends Actor {
    ...
    async updateLevel(newLevel) {
        ...
        if (newLevel > this.system.levelData.level.current) {
            ...
            if (newLevel > maxLevel) {
                ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.tooHighLevel'));
            }
            ...
            if (newLevel < 1) {
                ui.notifications.warn(game.i18n.localize('APP_EXAMPLE.UI.Notifications.tooLowLevel'));
            }
    ...
    async levelUp(levelupData) {
        ...
        if (this.system.companion && !this.system.companion.system.levelData.canLevelUp) {
            const confirmed = await foundry.applications.api.DialogV2.confirm({
                window: {
                    title: game.i18n.localize('APP_EXAMPLE.ACTORS.Character.companionLevelup.confirmTitle')
                },
                content: game.i18n.format('APP_EXAMPLE.ACTORS.Character.companionLevelup.confirmText', {
    ...
    async diceRoll(config) {
        ...
        const rollClass = config.roll.lite ? CONFIG.Dice["app-example"]['DHRoll'] : this.rollClass;
        return await rollClass.build(config);
    }

    get rollClass() {
        return CONFIG.Dice["app-example"][['character', 'companion'].includes(this.type) ? 'DualityRoll' : 'D20Roll'];
    }
    ...
    async importFromJSON(json) {
        ...
        if (foundry.utils.isNewerVersion('1.1.0', parsedJSON._stats.systemVersion)) {
            const confirmed = await foundry.applications.api.DialogV2.confirm({
                window: {
                    title: game.i18n.localize('APP_EXAMPLE.ACTORS.Character.InvalidOldCharacterImportTitle')
                },
                content: game.i18n.localize('APP_EXAMPLE.ACTORS.Character.InvalidOldCharacterImportText')
            });
            if (!confirmed) return;
        }
        ...
        return this.effects
            ...
            .reduce((acc, effect) => {
                ...
                const isAutoVulnerableEffect =
                    effect.flags["app-example"]?.autoApplyFlagId === conditions.vulnerable.autoApplyFlagId;
                if (isAutoVulnerableEffect) {
                    if (!autoVulnerableActive) return acc;

                    effect.appliedBy = game.i18n.localize('APP_EXAMPLE.CONFIG.Condition.vulnerable.autoAppliedByLabel');
