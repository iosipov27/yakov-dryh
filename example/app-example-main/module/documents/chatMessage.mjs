...
export default class DhpChatMessage extends foundry.documents.ChatMessage {
    ...
    async onApplyDamage(event) {
        ...
        if (targets.length === 0)
            return ui.notifications.info(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noTargetsSelectedOrPerm'));
    ...
    async onApplyEffect(event) {
        ...
        config.event = event;
        if (targets.length === 0)
            ui.notifications.info(game.i18n.localize('APP_EXAMPLE.UI.Notifications.noTargetsSelectedOrPerm'));
        this.consumeOnSuccess();
    ...
    clickTarget(event) {
        ...
        const token = canvas.tokens.get(event.currentTarget.dataset.token);
        if (!token) {
            ui.notifications.info(game.i18n.localize('APP_EXAMPLE.UI.Notifications.attackTargetDoesNotExist'));
            return;
        }
