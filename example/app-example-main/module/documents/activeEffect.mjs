...
export default class DhActiveEffect extends foundry.documents.ActiveEffect {
    ...
    get isSuppressed() {
        ...
        if (this.flags?.["app-example"]?.attachmentSource) {
            return false;
        }
    ...
    async _preCreate(data, options, user) {
        ...
        if (immuneStatuses.length > 0) {
            ...
            const scrollingTexts = immuneStatuses.map(status => ({
                text: game.i18n.format('APP_EXAMPLE.ACTIVEEFFECT.immuneStatusText', {
    ...
    _getTags() {
        const tags = [
            ...
            game.i18n.localize(
                this.isTemporary ? 'APP_EXAMPLE.EFFECTS.Duration.temporary' : 'APP_EXAMPLE.EFFECTS.Duration.passive'
            )
        ];
    ...
    async toChat(origin) {
        ...
        const msg = {
            title: game.i18n.localize('APP_EXAMPLE.GENERAL.Effect.single'),
            user: game.user.id,
            system: systemData,
            content: await foundry.applications.handlebars.renderTemplate(
                'systems/app-example/templates/ui/chat/action.hbs',
                systemData
            )
        };
