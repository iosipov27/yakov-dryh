...
export async function runMigrations() {
    ...
    if (foundry.utils.isNewerVersion('1.5.5', lastMigrationVersion)) {
        ...
        for (const scene of game.scenes) {
            if (!scene.flags["app-example"]) continue;
            const systemData = new game.system.api.data.scenes.DHScene(scene.flags["app-example"]);
            ...
            if (newEnvironments.length !== sceneEnvironments.length)
                await scene.update({ 'flags.app-example.sceneEnvironments': newEnvironments });
        }
