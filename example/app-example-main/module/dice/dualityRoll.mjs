...
export default class DualityRoll extends D20Roll {
    ...
    get title() {
        return game.i18n.localize(
            `APP_EXAMPLE.GENERAL.${this.options?.actionType === 'reaction' ? 'reactionRoll' : 'dualityRoll'}`
        );
    }
    ...
    get totalLabel() {
        const label = this.guaranteedCritical
            ? 'APP_EXAMPLE.GENERAL.guaranteedCriticalSuccess'
            : this.isCritical
              ? 'APP_EXAMPLE.GENERAL.criticalSuccess'
              : this.withHope
                ? 'APP_EXAMPLE.GENERAL.hope'
                : 'APP_EXAMPLE.GENERAL.fear';
            ...
            modifiers.unshift({
                label:
                    this.options.roll.type === CONFIG.DH.GENERAL.rollTypes.spellcast.id
                        ? 'APP_EXAMPLE.CONFIG.RollTypes.spellcast.name'
                        : `APP_EXAMPLE.CONFIG.Traits.${this.options.roll.trait}.name`,
