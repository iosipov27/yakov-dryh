...
CONFIG.ChatMessage.template = 'systems/app-example/templates/ui/chat/chat-message.hbs';
...
CONFIG.RollTable.resultTemplate = 'systems/app-example/templates/ui/chat/table-result.hbs';
...
CONFIG.ui.appExampleMenu = applications.sidebar.AppExampleMenu;
...
Hooks.once('init', () => {
    ...
    const sheetLabel = typePath => () =>
        game.i18n.format('APP_EXAMPLE.GENERAL.typeSheet', {
...
Hooks.on('renderChatMessageHTML', (document, element) => {
    ...
    const cssClass = document.flags?.["app-example"]?.cssClass;
...
Hooks.on('chatMessage', (_, message) => {
    if (message.startsWith('/dr')) {
        ...
        if (!result) {
            ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.dualityParsing'));
            return false;
        }
        ...
        const title =
            (flavor ?? traitValue)
                ? game.i18n.format('APP_EXAMPLE.UI.Chat.dualityRoll.abilityCheckTitle', {
                      ability: game.i18n.localize(SYSTEM.ACTOR.abilities[traitValue].label)
                  })
                : game.i18n.localize('APP_EXAMPLE.GENERAL.duality');
        ...
        enrichedDualityRoll({
            ...
            difficulty,
            title,
            label: game.i18n.localize('APP_EXAMPLE.GENERAL.dualityRoll'),
            actionType: null,
            advantage,
            grantResources
        });
    ...
    if (message.startsWith('/fr')) {
        ...
        if (!result) {
            ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.fateParsing'));
            return false;
        }
        ...
        if (!fateTypeData)
            return ui.notifications.error(game.i18n.localize('APP_EXAMPLE.UI.Notifications.fateTypeParsing'));

        const { value: fateType, label: fateTypeLabel } = fateTypeData;
        const target = getCommandTarget({ allowNull: true });
        const title = flavor ?? game.i18n.localize('APP_EXAMPLE.GENERAL.fateRoll');
